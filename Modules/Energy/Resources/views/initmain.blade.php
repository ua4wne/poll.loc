...
@section('content')
    ...
    <ul class="breadcrumb">
        ...
        <li><a href="{{ route('initmain') }}">Общие счетчики</a></li>
    ...
    <div class="x_content">
        ...
        <div class="row">
            <div class="alert alert-warning col-xs-offset-2 col-xs-8"><p class="text-center">Форма заполняется в случае установки нового счетчика!</p></div>
        </div>
        {!! Form::open(['url' => route('initmain'),'class'=>'form-horizontal','method'=>'POST','id'=>'new_val']) !!}

        <div class="form-group">
            {!! Form::label('ecounter_id', 'Счетчик:',['class'=>'col-xs-2 control-label']) !!}
            <div class="col-xs-8">
                {!! Form::select('ecounter_id', $selmain, old('ecounter_id'),['class' => 'form-control','required' => 'required','id'=>'ecounter_id']); !!}
            </div>
        </div>
