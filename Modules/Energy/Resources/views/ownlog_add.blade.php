...
@section('content')
    ...
    <div class="x_content">
        ...
        {!! Form::open(['url' => route('add_ownVal'),'class'=>'form-horizontal','method'=>'POST','id'=>'new_val']) !!}

        <div class="form-group">
            {!! Form::label('own_ecounter_id', 'Счетчик:',['class'=>'col-xs-2 control-label']) !!}
            <div class="col-xs-8">
                {!! Form::select('own_ecounter_id', $selown, old('own_ecounter_id'),['class' => 'form-control','required' => 'required','id'=>'own_ecounter_id']); !!}
            </div>
        </div>
